...
// MockStage is a test stage that implements the Stage interface
type MockStage struct {
	name        string
	inputTypes  []core.EventType
	outputTypes []core.EventType
}
...
func (m *MockStage) Process(ctx context.Context, input <-chan core.Event, output chan<- core.Event) error {
	return nil
}

func (m *MockStage) InputTypes() []core.EventType {
	return m.inputTypes
}

func (m *MockStage) OutputTypes() []core.EventType {
	return m.outputTypes
}

// TestGraphNodeCreation tests basic node creation and retrieval
...
// TestGraphDuplicateNode tests that duplicate nodes are rejected
...
// TestGraphEdgeCreation tests edge creation and filtering
...
// TestGraphEdgeNoFilter tests edge creation without filter (forward all)
func TestGraphEdgeNoFilter(t *testing.T) {
	graph := NewPipelineGraph()
...
// TestGraphEdgeInvalidNode tests edge creation with non-existent nodes
...
// TestGraphEntryNode tests entry node setting
...
// TestGraphExitNodes tests exit node management
func TestGraphExitNodes(t *testing.T) {
	graph := NewPipelineGraph()
	stage1 := &MockStage{name: "stage1"}
	stage2 := &MockStage{name: "stage2"}
...
// Graph builder rejects cycles
func TestPropertyGraphRejectsCycles(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		...
		if err == nil {
			rt.Fatalf("expected cycle detection error, got nil")
		}
	})
}

// Graph builder rejects unreachable stages
func TestPropertyGraphRejectsUnreachable(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		graph := NewPipelineGraph()
		...
		err := ValidateGraph(graph)
		if err == nil {
			rt.Fatalf("expected unreachable stage error, got nil")
		}
	})
}

// Graph builder constructs valid topology
func TestPropertyGraphConstructsValidTopology(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		graph := NewPipelineGraph()
		...
		err := ValidateGraph(graph)
		if err != nil {
			rt.Fatalf("valid graph failed validation: %v", err)
		}

		// Verify structure
		nodeA := graph.GetNode("A")
		if len(nodeA.Outputs()) != 1 {
			rt.Fatalf("node A should have 1 output, got %d", len(nodeA.Outputs()))
		}

		nodeB := graph.GetNode("B")
		if len(nodeB.Inputs()) != 1 {
			rt.Fatalf("node B should have 1 input, got %d", len(nodeB.Inputs()))
		}
		if len(nodeB.Outputs()) != 1 {
			rt.Fatalf("node B should have 1 output, got %d", len(nodeB.Outputs()))
		}
	})
}

// Type compatibility validation
func TestPropertyTypeCompatibilityValidation(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		graph := NewPipelineGraph()
		...
		err := ValidateGraph(graph)
		if err == nil {
			rt.Fatalf("expected type compatibility error, got nil")
		}
	})
}

// TestTypeCompatibilityWithWildcard tests that wildcard types are compatible
func TestTypeCompatibilityWithWildcard(t *testing.T) {
	graph := NewPipelineGraph()
...
// TestTypeCompatibilityWithFilter tests type compatibility with event filters
func TestTypeCompatibilityWithFilter(t *testing.T) {
	graph := NewPipelineGraph()
